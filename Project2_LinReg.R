...
# Try scaling the data first, maybe Z scaling (but not to Monthly Salary)

# Look at the p-values for original vars

# If you notice any data that's like 5 standard deviations away or more, maybe examine it
...
# start_num <- 3
start_num <- 1
vars_df <- add_vars_reg(data, all_vars, seed, prop_test, thresh, num_tests, num_no_change, num_models, start_formula, start_num)
...
write.xlsx(var_df,'C:/Users/aabro/OneDrive/Desktop/SMU Program/Doing data Science/Units/Unit 14 and 15 - Project 2 (Individual)/Excels/var_df.xlsx',colNames = TRUE)
...
head(preds)
head(as.numeric(data[,"MonthlyIncome"]))
err <- sqrt(sum((preds - as.numeric(data[,"MonthlyIncome"]))^2)/nrow(data)) # 1251
...
bstDMatrix <- xgboost(data = dtrain, nthread = 2, nrounds = 10, objective = "reg:squarederror")
...
rmse 
# It seems like it does a bad job of handling categorical data.  
...
start_formula <- ''
start_num <- 1
txt_file <- 'C:/Users/aabro/OneDrive/Desktop/SMU Program/Doing data Science/Units/Unit 14 and 15 - Project 2 (Individual)/Txt_Files/Save_Lin.txt'
start_best <- 1000000000
amount_to_improve <- 1
var_to_predict <- 'MonthlyIncome'
vars_df <- add_vars_reg(data, all_vars, seed, prop_test, thresh, num_tests, 
